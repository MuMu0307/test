...
public class UserCtrl {
    ...
    public Map<String,Object> unFriendUserOfRole(HttpServletRequest request,Integer roleId,Integer userId){
        ...
        try {
            ...
            returnMap.put("message", "异常：解除关系失败!");
            returnMap.put("success", false);
            e.printStackTrace();
        }
        return returnMap;
    ...
    @RequestMapping(value="/getScenicOnDuty",method= RequestMethod.POST)
    @ResponseBody
    public Map<String,Object> getScenicOnDuty(HttpServletRequest request,Integer id){
        Map<String,Object> returnMap = new HashMap<String,Object>();

        try {
            Map<String,Object> map = userService.getScenicOnDuty(id);
            returnMap.put("value", map.get("value"));
            returnMap.put("message", map.get("message"));
            returnMap.put("success", map.get("success"));
        } catch (Exception e) {
            returnMap.put("message", "异常!");
            returnMap.put("success", false);
            e.printStackTrace();
        }
        return returnMap;
