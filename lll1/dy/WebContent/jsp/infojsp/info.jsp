            ...
            <table class="table table-hover table-striped table-bordered">
                <caption>
                    <nav class="navbar navbar-default" role="navigation">
                        <div class="container-fluid">
                            <div class="navbar-header">
                                <a class="navbar-brand">文件列表</a>
                            </div>
                            <div>
                                <form class="navbar-form navbar-right" role="search"  onsubmit="return ;">
                                    <div class="form-group">
                                        <input type="text" class="form-control" placeholder="请输入文件名" ng-model="searchFileName">
                                    </div>
                                    <button type="button" class="btn btn-default" ng-click="actionOnFile(this,constRef[4][0])">
                                        <span class="glyphicon glyphicon-search"></span>&nbsp;<span ng-bind="constRef[4][0]"></span>
                                    </button>
                                    <button type="button" class="btn btn-default" ng-click="actionOnFile(this,constRef[4][1])">
                                        <span class="glyphicon glyphicon-plus"></span>&nbsp;<span ng-bind="constRef[4][1]"></span>
                                    </button>
                                </form>
                            </div>
                        </div>
                ...
                <thead>
                <tr><th>名称</th><th>后缀</th><th>创建时间</th><th>操作</th></tr>
                </thead>
                <tbody>
                <tr ng-repeat="item in fileList">
                    ...
                    <td ng-bind="item.createTime"></td>
                    <td><button ng-click="actionOnFile(item,constRef[4][2])" type="button" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-list-alt"></span>&nbsp;<span ng-bind="constRef[4][2]"></span></button>
                        <button ng-click="actionOnFile(item,constRef[4][3])" type="button" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-download-alt"></span>&nbsp;<span ng-bind="constRef[4][3]"></span></button>
                        <button ng-click="actionOnFile(item,constRef[4][4])" type="button" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-remove"></span>&nbsp;<span ng-bind="constRef[4][4]"></span></button></td>
                </tr>
                </tbody>
            </table>
            <div><!--分页bar ng-if则不能给button设置disabled因为ng-if不满足条件不会生成相应dom元素--><%--先放着--%>
